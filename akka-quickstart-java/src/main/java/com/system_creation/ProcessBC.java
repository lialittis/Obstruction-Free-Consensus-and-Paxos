...
public class ProcessBC extends UntypedAbstractActor {
	...
	static public class QuorumRequest{
		...
		public QuorumRequest(StampedValue val, int localseqnum) {
			this.val = val;
	...
	public void onReceive(Object msg) throws Exception {
		if (this.state != 2) { // If not faulty
			...
			if (msg instanceof Members) {
				// msg is all processes (*)
				this.mem = (Members) msg; // determine the system members
				this.localseqnum=this.id ; // this.id is not from msg, it's from this process (*)
				this.state = 3;
//				for(int x = 0; x <= mem.Nmembers - 1; x = x + 1) {
//				     mem.members.get(x).tell(new ReadRequest(localseqnum),getSelf()); // should change to QuoromRequest
//				     log.info("sent request number " + localseqnum);
//				}    
//				for(int x = 0; x < this.mem.members.size(); x = x + 1) {
//				log.info(this.mem.members.get(x)+": know member "+Integer.toString(x)); 
...
//			 }
			...
			else if (msg instanceof State) { // failure indication 
				...
				if (msg instanceof QuorumRequest) { 
					...
					log.info("the localseqnum received is : " + ((QuorumRequest)msg).localseqnum);
					log.info("the localseqnum sended is : " + new QuorumResponse(((QuorumRequest)msg).localseqnum).localseqnum);
					actorRef.tell(new QuorumResponse(((QuorumRequest)msg).localseqnum),getSelf()); // actorRef is the original sender of this received message(*)
				}
			else
...
//							 	    		req = new QuorumRequest(this.localseqnum);
					...
					&& this.state==3) { 
						...
						if (msgs.size()>=this.mem.members.size()/2+1) {
							...
							this.state=1;	// not waiting any longer
							log.info("P"+this.id+": completes operation "+this.localseqnum/2+": put"); // Q:why localseqnum is devided by 2
